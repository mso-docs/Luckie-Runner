...
- [ ] Move save/load snapshot logic into `ProgressManager` wrapping `SaveService` (build/apply snapshots, slot I/O).
- [ ] Shift audio UI/volume controls into `AudioController` bridging `AudioService` and DOM controls.
- [ ] Push dialogue/interaction handling fully into `UIManager/DialogueManager` (start/advance/hide bubble, NPC talk), with `Game` only routing signals.
